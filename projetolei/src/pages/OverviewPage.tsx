...
// ─── Sub-componentes ───────────────────────────────────────────────────────────
// Cada card é um componente separado por duas razões:
// 1. O OverviewPage fica legível — só vês a estrutura de layout, não os detalhes
// 2. Cada card pode ser reutilizado noutras páginas se necessário
...
function RecentAlertsCard() {
  ...
  const mockAlerts: AlertItem[] = [
    { id: '1', title: t('alerts.noActivity'), description: t('alerts.noMovement'), time: t('common.minutesAgo', { count: 10 }), severity: 'warning' },
    { id: '2', title: t('alerts.tempResolved'), description: t('alerts.tempResolvedDesc'), time: t('common.hoursAgo', { count: 2 }), severity: 'success' },
    { id: '3', title: t('alerts.dailyCheck'), description: t('alerts.dailyCheckDesc'), time: t('common.hoursAgo', { count: 5 }), severity: 'info' },
  ]
        ...
        {mockAlerts.map((alert) => (
          <div key={alert.id} className={`${severityStyles[alert.severity]} border rounded-[14px] px-4 py-4`}>
            <div className="flex items-start justify-between gap-2">
              ...
              <p className="text-xs text-[#6a7282] shrink-0">{alert.time}</p>
            </div>
